...
namespace GameSessionStatusMapper
{
    ...
    inline AWS_GAMELIFTSERVER_API GameSessionStatus GetGameSessionStatusForName(const std::string& s_name) {
      const char* name = s_name.c_str();
#else
    inline AWS_GAMELIFTSERVER_API GameSessionStatus GetGameSessionStatusForName(const char* name) {
#endif
...
#ifdef GAMELIFT_USE_STD
    inline AWS_GAMELIFTSERVER_API std::string GetNameForGameSessionStatus(GameSessionStatus value)
#else
    inline AWS_GAMELIFTSERVER_API const char* GetNameForGameSessionStatus(GameSessionStatus value)
#endif
    {
      switch (value) {
