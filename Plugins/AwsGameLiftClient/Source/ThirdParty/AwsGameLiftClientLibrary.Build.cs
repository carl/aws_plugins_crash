...
public sealed class AwsGameLiftClientLibrary : ModuleRules
{
	...
	public AwsGameLiftClientLibrary(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (SupportedPlatform)
		{
			...
			PublicDefinitions.Add("DISABLE_GAMELIFTCLIENT=0");
			...
			if (Target.Platform == UnrealTargetPlatform.Android)
			{
				...
				AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "LibAwsGameLiftClient_APL.xml"));
			}
		...
		{
			PublicDefinitions.Add("DISABLE_GAMELIFTCLIENT=1");
		}
	}
	...
	private void AddLibrary(string LibraryName) 
	{
		...
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			{
				Console.WriteLine("AwsGameLiftClientLibrary: Adding linux static lib: " + StaticLib);
				PublicAdditionalLibraries.Add(StaticLib);
			}
			else
			{
				Console.WriteLine("AwsGameLiftClientLibrary: Adding linux dynamic lib: " + BaseFile);
		...
		{
			throw new PlatformNotSupportedException("Platform " + Platform + " is not supported by the AwsGameLiftClient Module.");
		}
	}
}
