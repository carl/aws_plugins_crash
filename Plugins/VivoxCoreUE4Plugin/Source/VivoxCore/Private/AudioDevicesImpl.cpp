...
AudioDevicesImpl::AudioDevicesImpl(AudioDeviceType type): _noDevice(NULL_DEVICE_ID, NULL_DEVICE_ID, VivoxDeviceType::NullDevice), _systemDevice(SYSTEM_DEVICE_ID, SYSTEM_DEVICE_ID, VivoxDeviceType::DefaultSystemDevice),
                                                          _communicationDevice(COMMUNICATION_DEVICE_ID, COMMUNICATION_DEVICE_ID, VivoxDeviceType::DefaultCommunicationDevice), _volumeAdjustment(0), _muted(false)
{
    _type = type;
}
...
VivoxCoreError AudioDevicesImpl::SetActiveDevice(const IAudioDevice& device, const FOnSetActiveDeviceCompletedDelegate& theDelegate)
{
    ...
    FString virtualId = (static_cast<const AudioDeviceImpl&>(device).Type() == VivoxDeviceType::DefaultSystemDevice) ? SYSTEM_DEVICE_ID :
                 (static_cast<const AudioDeviceImpl&>(device).Type() == VivoxDeviceType::DefaultCommunicationDevice) ? COMMUNICATION_DEVICE_ID :
                 (static_cast<const AudioDeviceImpl&>(device).Type() == VivoxDeviceType::NullDevice) ? NULL_DEVICE_ID : device.Id();
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_capture_devices_t &r)
{
    ...
    _systemDevice = AudioDeviceImpl(UTF8_TO_TCHAR(r.default_capture_device->display_name), r.default_capture_device->device, VivoxDeviceType::DefaultSystemDevice);
    _communicationDevice = AudioDeviceImpl(UTF8_TO_TCHAR(r.default_communication_capture_device->display_name), r.default_communication_capture_device->device, VivoxDeviceType::DefaultCommunicationDevice);
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_render_devices_t &r)
{
    ...
    _systemDevice = AudioDeviceImpl(UTF8_TO_TCHAR(r.default_render_device->display_name), r.default_render_device->device, VivoxDeviceType::DefaultSystemDevice);
    _communicationDevice = AudioDeviceImpl(UTF8_TO_TCHAR(r.default_communication_render_device->display_name), r.default_communication_render_device->device, VivoxDeviceType::DefaultCommunicationDevice);
    if (_systemDevice.IsEmpty())
    {
