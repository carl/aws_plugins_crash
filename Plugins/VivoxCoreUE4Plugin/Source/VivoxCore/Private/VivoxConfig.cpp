...
void VivoxConfig::setCallbackHandle(void* callbackHandle)
{
    config.callback_handle = callbackHandle;
}

void VivoxConfig::SetOnAudioUnitBeforeRecvAudioMixedCallback(pf_on_audio_unit_before_recv_audio_mixed_t callback)
{
    config.pf_on_audio_unit_before_recv_audio_mixed = callback;
}

void VivoxConfig::SetOnAudioUnitAfterCapture(pf_on_audio_unit_after_capture_audio_read_t callback, pf_on_audio_unit_started_t audioStart, pf_on_audio_unit_stopped_t audioStop, pf_on_audio_unit_before_capture_audio_sent_t beforeAudioSent)
{
    if (callback)
    {
        config.pf_on_audio_unit_after_capture_audio_read = callback;
    }
    if (audioStart)
    {
        config.pf_on_audio_unit_started = audioStart;
    }
    if (audioStop)
    {
        config.pf_on_audio_unit_stopped = audioStop;
    }
    if (beforeAudioSent)
    {
        config.pf_on_audio_unit_before_capture_audio_sent = beforeAudioSent;
    }
   
}

void VivoxConfig::SetOnAudioUnitBeforeRecvAudioRenderCallback(pf_on_audio_unit_before_recv_audio_rendered_t callback)
{
    config.pf_on_audio_unit_before_recv_audio_rendered = callback;
}
