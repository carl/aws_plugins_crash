...
//pf_on_audio_unit_before_recv_audio_rendered_t ClientImpl::AudioBeforeRender(void* callbackHandle, const char* session_group_handle, const char* initial_target_uri, short* pcm_frames, int pcm_frame_rate, int audio_frame_rate, int channels_per_frame, int is_silence)
//{
//    
//    return pf_on_audio_unit_before_recv_audio_rendered_t();
//}
...
VivoxCoreError ClientImpl::SetAudioOutVolume(int volume)
{
    return VivoxNativeSdk::Get().SetAudioInputVolumeAdjustment((int32_t)(volume));
}

VivoxCoreError ClientImpl::SetAudioInVolume(int volume)
{   
    return VivoxNativeSdk::Get().SetAudioOutputVolumeAdjustment((int32_t)(volume));
}