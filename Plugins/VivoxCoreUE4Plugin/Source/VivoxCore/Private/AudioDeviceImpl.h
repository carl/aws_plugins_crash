 ...
 */
UENUM(BlueprintType)
enum class VivoxDeviceType : uint8
{
    /**
...
class AudioDeviceImpl : public IAudioDevice
{
    FString _name;
    FString _id;
    VivoxDeviceType _type;

public:
	AudioDeviceImpl();
    AudioDeviceImpl(const FString &name, const FString &id, VivoxDeviceType type);
    ~AudioDeviceImpl();
    const FString& Name() const override { return _name; }
    const FString& Id() const override { return _id; }
    VivoxDeviceType Type() const { return _type; } // no override: not in interface
