...
public sealed class AwsPollyLibrary : ModuleRules
{
	...
	public AwsPollyLibrary(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (SupportedPlatform)
		{
			...
			PublicDefinitions.Add("DISABLE_POLLY=0");
			...
			if (Target.Platform == UnrealTargetPlatform.Android)
			{
				...
				AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "LibAwsPolly_APL.xml"));
			}
		...
		{
			PublicDefinitions.Add("DISABLE_POLLY=1");
		}
	}
	...
	private void AddLibrary(string LibraryName) 
	{
		...
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			{
				Console.WriteLine("AwsPollyLibrary: Adding linux static lib: " + StaticLib);
				PublicAdditionalLibraries.Add(StaticLib);
			}
			else
			{
				Console.WriteLine("AwsPollyLibrary: Adding linux dynamic lib: " + BaseFile);
		...
		{
			throw new PlatformNotSupportedException("Platform " + Platform + " is not supported by the AwsPolly Module.");
		}
	}
}
